...
│    │  │    :8080  ◄──────┼───┼── :8080          │   │  :5128 (REST)    │    │
│    │  │  (對外 5010:8080) │   │  (auth_db:5432)  │   │  :5129 (gRPC)    │    │
│    │  └────────┬─────────┘   └──────────────────┘   └──────────────────┘    │
│    │           │                                                            │
│    │           │ 路由轉發                                                    │
│    │           ├──► auth-service:8080                                       │
...
│    │                                                                        │
│    │  ┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐    │
│    │  │ realtime-service │   │  grpc-service    │   │    worker        │    │
│    │  │    :8080         │   │    :8080         │   │  （無對外 port）   │    │
│    │  └──────────────────┘   └──────────────────┘   └──────────────────┘    │
│    │                                                                        │
│    │  ┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐    │
│    │  │   tcp-service    │   │    postgres      │   │    rabbitmq      │    │
│    │  │ :5400 ◄──────────┼─┐ │    :5432         │   │  :5672 (AMQP)    │    │
│    │  │  (對外 5400:5400)│  │ │  (postgres-data) │   │  :15672 (UI)     │    │
│    │  └──────────────────┘ │ └──────────────────┘   └──────────────────┘    │
│    │                       │                                                │
│    │  ┌──────────────────┐ │ ┌──────────────────┐   ┌──────────────────┐    │
│    │  │    jaeger        │ │ │   prometheus     │   │    grafana       │    │
│    │  │  :4317 (OTLP)    │ │ │    :9090         │   │    :3000         │    │
│    │  │  :16686 (UI) ◄───┼─┘ │  (對外 9090:9090) │   │  (對外 3000:3000) │   │
│    │  │  (對外 16686)     │   │                  │   │                  │    │
│    │  └──────────────────┘   └──────────────────┘   └──────────────────┘    │
│    └────────────────────────────────────────────────────────────────────────┘
```
...
```
本機（Mac）→ 只能連到有 ports: 設定的服務（gateway:5010、tcp-service:5400 等）
Docker 內部 → 任何服務都能用名稱連到任何服務（不需要 ports:）
```
